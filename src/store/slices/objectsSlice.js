...
export const createObjectsSlice = (set, get) => ({
  ...
  addFacility: (category, floorId) => {
    ...
    const newObject = {
      ...
      verticalHeightMm: template.verticalHeightMm || DEFAULT_VERTICAL_HEIGHT_MM,
      elevationMm: template.elevationMm || 0,
      memo: '',
      rotation: 0,
      flipX: false,
      flipY: false,
  ...
  updateSelectionColor: (ids, fill) => {
    get().pushHistorySnapshot();
    const idSet = new Set(ids);
    set({
      objects: get().objects.map((o) => (idSet.has(o.id) ? { ...o, fill } : o)),
    });
  },

  updateSelectionSize: (ids, widthM, heightM) => {
    get().pushHistorySnapshot();
    const idSet = new Set(ids);
    set({
      objects: get().objects.map((o) => (
        idSet.has(o.id) ? { ...o, width: widthM, height: heightM } : o
      )),
    });
  },
